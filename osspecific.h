...
//
// GetSecurityDescriptorOwner. Only supported on the NT platform
//
class CSecurityApi
{
public:
	...
	BOOL IsOwnedBy(LPCTSTR lpFileName, PSID lpSid);
	BOOL IsOwnedBy(LPCTSTR lpFileName, CList<PSID> &sids);

private:
	typedef BOOL (WINAPI *TypeGetFileSecurity)(LPCTSTR lpFileName, SECURITY_INFORMATION RequestedInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor, DWORD nLength, LPDWORD lpnLengthNeeded);
	typedef BOOL (WINAPI *TypeGetSecurityDescriptorOwner)(PSECURITY_DESCRIPTOR pSecurityDescriptor, PSID* pOwner, LPBOOL lpbOwnerDefaulted);
	typedef BOOL (WINAPI *TypeEqualSid)(PSID pSid1, PSID pSid2);

	HMODULE m_dll;
	bool m_UnloadDll;
	
	TypeGetFileSecurity m_GetFileSecurity;
	TypeGetSecurityDescriptorOwner m_GetSecurityDescriptorOwner;
	TypeEqualSid m_EqualSid;
};
