...
CSecurityApi::CSecurityApi()
	: m_UnloadDll(false)
{
	m_dll = GetModuleHandle(_T("advapi32.dll"));
	if(!m_dll)
	{
		m_dll = LoadLibrary(_T("advapi32.dll"));
		m_UnloadDll = (m_dll != NULL);
	}

	TGETPROC(GetFileSecurity);
	GETPROC(GetSecurityDescriptorOwner);
	GETPROC(EqualSid);
}
...
bool CSecurityApi::IsSupported()
{
	CHECK(GetFileSecurity);
	CHECK(GetSecurityDescriptorOwner);
	CHECK(EqualSid);
	return true;
}

BOOL CSecurityApi::IsOwnedBy(LPCTSTR lpFileName, PSID lpSid)
{
	ASSERT(IsSupported());
	...
	sids.AddHead(lpSid);
	return IsOwnedBy(lpFileName, sids);
}

BOOL CSecurityApi::IsOwnedBy(LPCTSTR lpFileName, CList<PSID> &sids)
{
	ASSERT(IsSupported());
	...
	if (m_GetFileSecurity(lpFileName, OWNER_SECURITY_INFORMATION, NULL, 0, &size) || GetLastError() != ERROR_INSUFFICIENT_BUFFER)
		return FALSE;
	sd = (PSECURITY_DESCRIPTOR)HeapAlloc(GetProcessHeap(), 0, size);
	if (sd == NULL)
		return FALSE;
	if (m_GetFileSecurity(lpFileName, OWNER_SECURITY_INFORMATION, sd, size, &size) && m_GetSecurityDescriptorOwner(sd, &sid, &defaulted))
	{
		POSITION pos = sids.GetHeadPosition();
		while (pos)
		{
			if (m_EqualSid(sid, sids.GetNext(pos)))
			{
				result = TRUE;
				break;
			}
